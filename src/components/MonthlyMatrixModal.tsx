...
export const MonthlyMatrixModal: React.FC<MonthlyMatrixModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/80 backdrop-blur-xs p-2 sm:p-6 overflow-hidden">
      <div className="w-full max-w-6xl bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden my-auto max-h-[92vh] flex flex-col">
        {/* Header */}
        <div className="p-3.5 sm:p-5 bg-slate-900 text-white flex items-center justify-between gap-3 shrink-0">
          <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
            <div className="p-2 sm:p-2.5 bg-blue-600 rounded-xl shrink-0">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[10px] sm:text-xs text-slate-300 truncate">
                Matrix view of all records for {getMonthDisplayName(selectedMonthKey)}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 py-1.5 sm:px-3 text-xs font-bold bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg transition-all cursor-pointer"
            >
        ...
        <div className="p-3 bg-slate-50 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-2.5 shrink-0">
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <span className="text-sm sm:text-base font-bold text-slate-900 dark:text-white font-mono px-2 sm:px-3">
          ...
          <div className="flex items-center gap-3 sm:gap-4 text-[11px] sm:text-xs font-bold flex-wrap justify-center">
            <div className="flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded bg-emerald-500 inline-block"></span>
              <span className="text-slate-700 dark:text-slate-300">P = Present</span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded bg-red-500 inline-block"></span>
              <span className="text-slate-700 dark:text-slate-300">A = Absent</span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded bg-purple-500 inline-block"></span>
        ...
        <div className="overflow-auto flex-1 p-2 sm:p-4">
          <table className="w-full text-center border-collapse text-xs">
            <thead>
              <tr className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold uppercase sticky top-0 z-20">
                <th className="p-2.5 text-left border border-slate-200 dark:border-slate-700 min-w-[120px] sm:min-w-[140px] sticky left-0 z-30 bg-slate-100 dark:bg-slate-800">
                  Student Name
                </th>
                <th className="p-2 text-center border border-slate-200 dark:border-slate-700 min-w-[50px] sticky left-[120px] sm:left-[140px] z-30 bg-slate-100 dark:bg-slate-800">
                  Class
                </th>
                {daysArray.map((day) => (
                  <th
              ...
              {students.map((student) => {
                ...
                return (
                  <tr key={student.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                    <td className="p-2.5 text-left font-bold text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700 whitespace-nowrap sticky left-0 z-10 bg-white dark:bg-slate-900">
                      {student.name}
                    </td>
                    <td className="p-2 border border-slate-200 dark:border-slate-700 font-semibold text-slate-600 dark:text-slate-400 sticky left-[120px] sm:left-[140px] z-10 bg-white dark:bg-slate-900">
