...
import { X, Calendar as CalendarIcon, Download, Edit3, ChevronLeft, ChevronRight } from 'lucide-react';
...
export const StudentDetailModal: React.FC<StudentDetailModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/80 backdrop-blur-xs p-2 sm:p-6 overflow-y-auto">
      <div className="w-full max-w-4xl bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden my-auto max-h-[95vh] sm:max-h-[92vh] flex flex-col">
        
        {/* Modal Top Bar */}
        <div className="p-4 sm:p-6 bg-[#1E3A8A] text-white flex items-center justify-between shrink-0">
          <div className="pr-2">
            <div className="flex flex-wrap items-center gap-2 sm:gap-3">
              <h2 className="text-lg sm:text-2xl font-bold tracking-tight">{student.name}</h2>
              <span className="px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full text-[10px] sm:text-xs font-bold bg-[#FCD34D] text-[#1E3A8A]">
          ...
          <button
            onClick={onClose}
            className="p-1.5 sm:p-2 text-blue-200 hover:text-white bg-white/10 hover:bg-white/20 rounded-xl transition-colors cursor-pointer shrink-0"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-2 sm:p-4 bg-slate-50 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 shrink-0">
          ...
          <div className="flex items-center justify-end gap-2 shrink-0">
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 text-xs font-bold text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg hover:bg-slate-100 transition-all cursor-pointer shadow-2xs"
            >
              <Download className="w-3.5 h-3.5 text-[#1E3A8A]" />
              <span className="hidden xs:inline">Download Today PDF</span>
              <span className="xs:hidden">Today PDF</span>
            </button>
            <button
              onClick={() => generateStudentPDF(student, selectedMonthKey)}
              className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 text-xs font-extrabold text-[#1E3A8A] bg-[#FCD34D] hover:bg-amber-400 rounded-lg transition-all cursor-pointer shadow-xs"
            >
              <Download className="w-3.5 h-3.5" />
              <span className="hidden xs:inline">Download Monthly PDF</span>
              <span className="xs:hidden">Monthly PDF</span>
            </button>
            <button
        ...
        <div className="p-3 sm:p-6 overflow-y-auto space-y-6 flex-1">
          ...
          {activeTab === 'Overview' && (
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between bg-[#F8FAFC] dark:bg-slate-800/80 p-3.5 rounded-xl border border-slate-200 dark:border-slate-700">
                <div>
                  <span className="text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-wider">Viewing Month</span>
                  <div className="text-sm sm:text-base font-extrabold text-[#1E3A8A] dark:text-blue-300">
                ...
                <div className="flex items-center gap-1.5 sm:gap-2">
                  ...
                  <span className="text-xs font-bold font-mono px-1 sm:px-2 text-slate-700 dark:text-slate-300">
                    {selectedMonthKey}
                  </span>
                  <button
              ...
              <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-4 sm:p-5 shadow-2xs">
                <h3 className="text-xs sm:text-sm font-bold text-[#1E3A8A] dark:text-blue-300 uppercase tracking-wider mb-4">
                  Month Attendance Summary ({getMonthDisplayName(selectedMonthKey)})
                </h3>

                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                  <div className="bg-[#DEF7EC] p-3 sm:p-4 rounded-xl border border-emerald-200 text-center">
                    <span className="text-[11px] sm:text-xs font-bold text-[#03543F]">Present Days</span>
                    <div className="text-xl sm:text-2xl font-black text-[#059669] mt-1">
                      {currentMonthStats.present}
                    </div>
                  </div>

                  <div className="bg-[#FDF2F2] p-3 sm:p-4 rounded-xl border border-red-200 text-center">
                    <span className="text-[11px] sm:text-xs font-bold text-[#9B1C1C]">Absent Days</span>
                    <div className="text-xl sm:text-2xl font-black text-[#DC2626] mt-1">
                      {currentMonthStats.absent}
                    </div>
                  </div>

                  <div className="bg-purple-50 dark:bg-purple-950/40 p-3 sm:p-4 rounded-xl border border-purple-200/60 text-center">
                    <span className="text-[11px] sm:text-xs font-bold text-purple-800 dark:text-purple-300">Holidays</span>
                    <div className="text-xl sm:text-2xl font-black text-[#6366F1] mt-1">
                      {currentMonthStats.holiday}
                    </div>
                  </div>

                  <div className="bg-blue-50 dark:bg-blue-950/40 p-3 sm:p-4 rounded-xl border border-blue-200/60 text-center flex flex-col justify-center items-center">
                    <span className="text-[11px] sm:text-xs font-bold text-[#1E3A8A] dark:text-blue-300">Attendance %</span>
                    <div className="text-xl sm:text-2xl font-black text-[#1E3A8A] dark:text-blue-400 mt-1">
          ...
          {(activeTab === 'Calendar' || activeTab === 'Overview') && (
            <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3 sm:p-5 shadow-2xs">
              <div className="flex items-center justify-between gap-2 mb-4">
                <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white flex items-center gap-2">
                  <CalendarIcon className="w-4 h-4 sm:w-5 sm:h-5 text-[#1E3A8A]" />
                  <span className="truncate">Calendar - {getMonthDisplayName(selectedMonthKey)}</span>
                </h3>

                <div className="flex items-center gap-1 sm:gap-2 shrink-0">
                  <button
                    onClick={handlePrevMonth}
                    className="p-1 sm:p-1.5 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg text-slate-600 dark:text-slate-300 cursor-pointer"
                  >
              ...
              <p className="text-[11px] sm:text-xs text-slate-500 dark:text-slate-400 mb-4 bg-slate-50 dark:bg-slate-900 p-2.5 rounded-xl border border-slate-200 dark:border-slate-800">
              ...
              <div className="overflow-x-auto">
                <div className="min-w-[320px]">
                  ...
                  <div className="grid grid-cols-7 gap-1 sm:gap-1.5 text-center text-xs font-bold text-slate-500 mb-2">
                  ...
                  <div className="grid grid-cols-7 gap-1 sm:gap-1.5">
                    {Array.from({ length: firstDayOfWeek }).map((_, idx) => (
                      <div key={`blank-${idx}`} className="h-12 sm:h-14 bg-slate-50/50 dark:bg-slate-900/30 rounded-lg sm:rounded-xl" />
                    ))}
                    ...
                    {daysArray.map((day) => {
                      ...
                      return (
                        <div
                          key={fullDateStr}
                          className={`h-14 sm:h-16 p-1 sm:p-1.5 rounded-lg sm:rounded-xl border flex flex-col justify-between items-center transition-all cursor-pointer group min-w-[38px] ${
                        ...
                        >
                          <div className="w-full flex items-center justify-between text-[10px] sm:text-[11px] font-bold leading-none">
                            <span className={isSunday ? 'text-red-600 font-extrabold' : ''}>{day}</span>
                            {existingRecord?.isOverride && (
                              <span className="text-[8px] sm:text-[9px] bg-amber-500 text-white px-0.5 sm:px-1 rounded">★</span>
                            )}
                          </div>

                          <div className="w-full text-center text-[8px] sm:text-[10px] font-extrabold uppercase tracking-tight truncate">
                            {currentStatus || 'Mark'}
                          </div>
                        </div>
                      );
          ...
          {/* FEES TAB */}
          {activeTab === 'Fees' && (
            <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-4 sm:p-5 shadow-2xs space-y-4">
